...
class EpubBuilder {
  ...
  void _prepareResources() {
    ...
    // 1. 样式表
    _resources['OEBPS/css/styles.css'] = _createStylesheet();
    ...
    // 6. 封面XHTML文件
    _resources['OEBPS/cover.xhtml'] = _createCoverXhtml();
  }

  /// 创建封面XHTML文件
  String _createCoverXhtml() {
    ...
    builder.element(
      ...
      nest: () {
        ...
        builder.element(
          ...
          nest: () {
            // 章节标题
            builder.element('h2', nest: chapter.title);
      ...
      nest: () {
        ...
        builder.element(
          ...
          nest: () {
            ...
            builder.element(
              'navPoint',
              attributes: {'id': 'navpoint-cover', 'playOrder': '1'},
              nest: () {
                builder.element(
                  'navLabel',
                  nest: () {
                    builder.element('text', nest: '封面');
                  },
                );
                builder.element(
                  'content',
                  attributes: {'src': 'cover.xhtml'},
                );
              },
            );

            for (int i = 0; i < chapters.length; i++) {
              final playOrder = i + 2; // 封面占用 playOrder=1，章节从2开始
              builder.element(
                'navPoint',
                attributes: {
